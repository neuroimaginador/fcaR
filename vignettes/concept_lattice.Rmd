...
For a `ConceptLattice`, one may want to retrieve particular concepts (function `get_concepts_by_id()`):
```{r}
fc_planets$concepts$get_concepts_by_id(2:3)
```

Or get all the extents and all the intents of all concepts, as sparse matrices:
```{r eval = FALSE}
fc_planets$concepts$get_extents()
fc_planets$concepts$get_intents()
```

## Concept support
The support of concepts can be computed using the function `compute_support()`:
```{r}
fc_planets$concepts$compute_support()
```

## Sublattices

When the concept lattice is too large, it can be useful in certain occasions to just work with a sublattice of the complete lattice. To this end, we use the `get_sublattice()` function.
...
idx <- which(fc_I$concepts$compute_support() > 0.2)
# Build the sublattice
sublattice <- fc_I$concepts$get_sublattice(idx)
sublattice

```

And we can plot just the suublattice:
```{r}
sublattice$plot()
```
...
```{r}
# The fifth concept
C <- fc_planets$concepts$get_concepts_by_id(5)[[1]]
C
# Its subconcepts:
fc_planets$concepts$get_subconcepts(C)
# And its superconcepts:
fc_planets$concepts$get_superconcepts(C)
```
...
```{r}
# A list of concepts
C <- fc_planets$concepts$get_concepts_by_id(5:7)
C
