...
test_that("Factorization works for fuzzy contexts and recovers dimensions", {
  ...
  I_fuzzy <- matrix(c(0.9, 0.1, 0.8,
                      0.2, 1.0, 0.0,
                      0.8, 0.0, 0.9), nrow = 3, byrow = TRUE)
  ...
  res_grecond <- fc$factorize(method = "GreConD", w = 0.5, stop_threshold_ratio = 0.1)
  ...
  expect_equal(res_grecond$object_factor$dim()[2], res_grecond$factor_attribute$dim()[1]) # k == k
  ...
  expect_error(fc$factorize(method = "UnknownMethod"), "Unknown factorization method")
  ...
  expect_error(fc_empty$factorize(method = "GreConD"), NA)
})
