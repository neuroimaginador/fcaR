...
struct Node {
  ...
  std::vector<int> predecessors; // Vecinos inferiores
  std::vector<int> successors;   // Vecinos superiores

  Node() : id(0), grade(-1), x_coord(0.0), y_coord(0.0) {}
  Node(int _id) : id(_id), grade(-1), x_coord(0.0), y_coord(0.0) {}
  Node(int _id, int _grade, double _x) : id(_id), grade(_grade), x_coord(_x), y_coord((double)_grade) {}
};

struct BaricenterComparator {
  const std::map<int, double>& baricenter_values;
  BaricenterComparator(const std::map<int, double>& values) : baricenter_values(values) {}
    ...
    const IntegerVector& edge_to)
{
  ...
  while (!q.empty()) {
    ...
    for (int v_id : nodes_map.at(u_id).successors) {
      ...
      in_degree.at(v_id)--;
      if (in_degree.at(v_id) == 0) q.push(v_id);
    }
  }

  if (processed_count < N) {
    for (int id : concept_ids) {
      if (nodes_map.at(id).grade == -1) nodes_map.at(id).grade = 0;
    }
  }
...
// --- A. SUGIYAMA FRAMEWORK MEJORADO (Centrado) ---
void apply_sugiyama_layout(std::map<int, Node>& nodes_map, const IntegerVector& concept_ids, int max_grade) {
  ...
  // Colocamos cada nodo en su índice, pero luego restamos la mitad del ancho
  for (auto& layer : layers) {
    double width = layer.size();
    for (size_t i = 0; i < width; ++i) {
      // Ejemplo ancho 3: Indices 0, 1, 2 -> Coords -1, 0, 1
      nodes_map.at(layer[i]).x_coord = (double)i - (width - 1.0) / 2.0;
    }
  }

  // 2. Minimización de Cruces + Alineamiento Vertical (Baricentro)
  int iterations = 20;

  for (int iter = 0; iter < iterations; ++iter) {

    // Top-Down (Mejorar posiciones X basándose en padres/predecesores)
    for (int k = 1; k <= max_grade; ++k) {
      ...
      for (int u_id : current_layer) {
        ...
        // Si tiene padres, intentamos ponernos debajo de su promedio
        // Si no, nos quedamos donde estamos (para mantener el centrado inicial)
        baricenters[u_id] = (count > 0) ? sum_x / count : nodes_map.at(u_id).x_coord;
      }

      // Ordenamos la capa por estas posiciones ideales
      std::sort(current_layer.begin(), current_layer.end(), BaricenterComparator(baricenters));

      // REASIGNACIÓN CENTRADA:
      // Una vez ordenados para minimizar cruces, volvemos a distribuirlos uniformemente
      // centrados en el eje 0, para evitar que se acumulen.
      double width = current_layer.size();
      for (size_t i = 0; i < width; ++i) {
        // Hacemos un mix entre la posición entera rígida (para evitar solapamientos)
        // y la posición baricéntrica (para alineación visual).
        // Aquí usamos posición rígida para garantizar limpieza como hasseDiagram.
        nodes_map.at(current_layer[i]).x_coord = (double)i - (width - 1.0) / 2.0;
      }
    }

    // Bottom-Up (Mejorar posiciones X basándose en hijos/sucesores)
  ...
  // Finalizar coordenadas Y
  for (auto& pair : nodes_map) {
    pair.second.y_coord = (double)pair.second.grade;
  }
}

// --- B. FORCE-DIRECTED (Orgánico) ---
void apply_force_directed_layout(std::map<int, Node>& nodes_map, const IntegerVector& concept_ids) {
  int N = concept_ids.size();
  double area = N * N;
...
// [[Rcpp::export]]
DataFrame calculate_lattice_layout_rcpp(
    const IntegerVector& concept_ids,
