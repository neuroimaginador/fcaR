...
// =============================================================================
  // --- HELPERS DE E/S A GRANEL (Dependencias para v_reorder) ---
  // =============================================================================
  ...
  // (Helper para el bitset de 'intent' plano durante el desempaquetado)
  ...
  */
     ...
     * Esta es la implementación más rápida, combinando E/S amortizada,
     * estructura de datos transpuesta nativa y reordenación de atributos.
     */
    // [[Rcpp::export]]
    List InClose_Reorder(NumericMatrix I,
      ...
      // 1. *** CÁLCULO DE SOPORTE Y REORDENACIÓN ***
      ...
      // 4. *** COMPROBACIÓN DEL CONCEPTO ÍNFIMO (BOTTOM) ***
      ...
      if (m_prime_is_empty) {
        ext_p_out.push_back(ext_i_out.size()); // size() es 0
      ...
      // 7. EMPAQUETADO A GRANEL (¡CON DE-REORDENACIÓN!)

      // --- ¡¡LA CORRECCIÓN ESTÁ AQUÍ!! ---
      SparseVector extents_out;
      initVector(&extents_out, n_objects); // <-- ANTES ERA 0
      // ---
      ...
      // (El empaquetado de Intents no cambia)
