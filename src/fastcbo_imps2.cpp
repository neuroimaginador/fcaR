...
void setdifference_fcbo2(SparseVector x,
                        SparseVector y,
                        SparseVector *res) {
...
void semantic_closure_fcbo2(SparseVector A,
                           ImplicationTree t,
                           SparseVector LHS,
                           SparseVector RHS,
                           SparseVector *res) {
...
void FuzzyFastGenerateFrom4_imps2(double* I,
  ...
  SparseVector premise, closure, diff;
  ...
  if ((cardinal(B) < n_attributes) && (y < n_attributes)) {
    ...
    for (int j = n_attributes - 1; j >= y; j--) {
      ...
      for (int g_idx = 0; g_idx < n_grades; g_idx++) {
        ...
        semantic_closure_fcbo2(pr, *tree, *LHS, *RHS, &closure);

        reinitVector(&diff);

        setdifference_fcbo2(D, closure, &diff);
        ...
        if (cardinal(diff) > 0) {
          ...
          //
          //   Rcout << "Implication rejected" << std::endl;
          //   printImpl(closure, diff, attrs);
          //
          //   Rcout << "Premise: " << std::endl;
          //   printArray(pr.i);
  ...
  for (int cols = 0; cols < queue_y.used; cols++) {
    ...
    FuzzyFastGenerateFrom4_imps2(I,
                                myI,
                                n_objects,
                                n_attributes,
                                n_grades,
...
// [[Rcpp::export]]
List FuzzyFastCbo_C4_imps2(NumericMatrix I,
                          StringVector attrs,
  ...
  FuzzyFastGenerateFrom4_imps2(I.begin(),
                              myI,
                              n_objects,
                              n_attributes,
                              n_grades,
