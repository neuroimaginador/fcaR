...
- **Tidyverse integration:** Implemented S3 methods to support `dplyr`
  ...
  objects:
  - **FormalContext:** Support for `select()` (attributes),
    [`filter()`](https://rdrr.io/r/stats/filter.html) (objects),
    `mutate()` (feature engineering), `arrange()` (sorting), and
    `rename()`. Includes support for `tidyselect` helpers (e.g.,
    `starts_with()`).
  - **ImplicationSet:** Support for
    [`filter()`](https://rdrr.io/r/stats/filter.html) (based on metrics
    or attributes), `arrange()` (sorting rules), and `slice()`
    (subsetting by index).
